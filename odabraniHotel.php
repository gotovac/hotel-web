...
if(isset($_GET["id"])){
    ...
    $sqlSobe = "SELECT id_soba, broj_sobe, tip_sobe, status, cijena, broj_rez FROM sobe WHERE sobe.hoteli_id_hotel = '" . $idHotel . "' GROUP BY id_soba";
    $sobe = $baza->selectDB($sqlSobe);

    $head = "<thead>" . "<tr>" . "<th>Tip</th>" . "<th>Broj sobe</th>" . "<th>Status</th>" . "<th>Cijena</th>" . "<th>Broj rezervacija</th>" . "</tr>" . "</thead>";
    $table = "";

    while ($row = $sobe->fetch_assoc()) {
        $table = $table . "<tr> <td>" . $row["tip_sobe"] . "</td>" . "<td>" . $row["broj_sobe"] . "</td>" . "<td>" . $row["status"] . "</td>" . "<td>" . $row["cijena"] . "</td>" . 
            "<td>" . $row["broj_rez"] . "</td> </tr>";
